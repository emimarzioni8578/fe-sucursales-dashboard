...
export class CalidadDatosComponent {
  ...
  getCoberturaData(d: any): ChartData<'bar'> {
    return {
      labels: d.provincias.map((p: any) => p.nombre),
      datasets: [
        { label: 'Cobertura Social (%)', data: d.provincias.map((p: any) => p.pctCoberturaSocial), backgroundColor: '#ab47bc' },
        { label: 'Cobertura Distribuidor (%)', data: d.provincias.map((p: any) => p.pctCoberturaDist), backgroundColor: '#26a69a' },
      ],
    };
  }
}
